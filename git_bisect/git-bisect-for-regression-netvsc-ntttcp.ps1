...
function RunBenchmarking( [string]$logid, [string]$bisect_commit_id )
{
    ...
    echo "ntttcp -r -P 1 -D" >  .\run-ntttcp.sh
    ...
    echo "ntttcp -s$server_VM_ip -P 1 -n 1 > $logFile"  > .\run-ntttcp.sh
    ...
    echo "$(get-date -f "yyyy-MM-dd HH:mm")    $logid $bisect_commit_id $thisResult" >> 00-git-bisect-status.log
