...
function SendCommandToVM([String] $ipv4, [String] $sshKey, [string] $command)
{
    ...
    $process = Start-Process bin\plink -ArgumentList "-i ssh\${sshKey} root@${ipv4} ${command}" -PassThru -NoNewWindow -redirectStandardOutput lisaOut.tmp -redirectStandardError lisaErr.tmp
    if ($process.ExitCode -eq 0)
    {
        $retVal = $True
    }
...
function SendFileToVM([String] $ipv4, [String] $sshkey, [string] $localFile, [string] $remoteFile, [Switch] $ChangeEOL)
{
    ...
    if (-not $ChangeEOL)
    {
        bin\plink -i ssh\${sshKey} root@${ipv4} "dos2unix -q $remoteFile"
    }
